...
tx = sprintf('%d %s', dx, options.XUnits); % time scalebar text
ty = sprintf('%d %s', dy, options.YUnits); % vertical scalebar text
...
h.YText = text(ax, x0-0.05*dx, y0+dy/2, ty, 'FontName','Tahoma','HorizontalAlignment','right','Color',options.Color);
h.XText = text(ax, x0+dx/2, y0-0.05*dy, tx, 'FontName','Tahoma','HorizontalAlignment','center','VerticalAlignment','top','Color',options.Color);
