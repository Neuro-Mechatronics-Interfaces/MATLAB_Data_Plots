function fig = plot_emg_averages(SUBJ, YYYY, MM, DD, ARRAY, BLOCK, varargin)
%PLOT_EMG_AVERAGES  Handle plotting gridded averages (TMSi/HD-EMG array)
%
% Syntax:
%   fig = plot_emg_averages(SUBJ, YYYY, MM, DD, ARRAY, BLOCK);
%   fig = plot_emg_averages(___, 'Name', value,...);
%
% Example 1:
%   % Run a single figure with default parameters.
%   fig = plot_emg_averages('Frank', 2021, 11, 18, "A", 43);
%   
% Example 2:
%   % Export a batch run of average figures.
%   plot_emg_averages('Frank', 2021, 11, 18, ["A", "B"], 0:105);
%
% Example 3:
%   % Modify specific parameters for a figure.
%   plot_emg_averages('Frank', ...
%       2021, 11, 18, "A", 43, ...
...
switch pars.EMG_Type
    ...
    case 'Bipolar'
        fig = plot_emg_averages__bipolar(SUBJ, YYYY, MM, DD, ARRAY, BLOCK, pars);
    otherwise
