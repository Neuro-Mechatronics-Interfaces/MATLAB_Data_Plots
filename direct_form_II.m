function [fig, handles] = direct_form_II(b, a, options)
%DIRECT_FORM_II Plot Direct Form-II implementation block diagram and pole/zero plot.
%
% Syntax:
%   [fig, handles] = plot.direct_form_II(b,a,'Name',value,...);
%
% Example:
%   [b,a] = butter(2,0.1,'high');
%   fig = plot.direct_form_II(b,a);
%
...
% The Direct-Form-II implementation is more-efficient than the equivalent 
% Direct-Form-I realization of the same filter due to using half the number
% of "delay" blocks (which correspond to real hardware in digital circuits,
% or presumably more neurons in a neural one). However, this form can
% suffer from numerical-instability issues related to the precision of the 
% values of the poles (denominator coefficients). 
%
% Inputs:
...
%   'Transposed' (1,1) logical = false -- Set true to plot transposed Direct-Form-II implementation. 
...
%
% See also: Contents, plot.direct_form_I
...
if isempty(options.Parent)
    fig = figure('Name', 'Direct Form-II Filter Implementation', ...
        'Color', 'w', 'Position', [200 270 800 720]);
    handles.layout = tiledlayout(fig, 2, 3);
    if options.Transposed
        title(handles.layout,'Transposed Direct Form-II Filter Characteristics', ...
            'FontName','Tahoma','Color',[0.65 0.65 0.65], 'FontWeight','bold');
    else
        title(handles.layout,'Direct Form-II Filter Characteristics', ...
            'FontName','Tahoma','Color',[0.65 0.65 0.65], 'FontWeight','bold');
    end
else
...
if ~options.Transposed
    ...
    for ii = 1:numel(b)
        ...
        if any(abs(b(ii:end)) > options.CoefficientThreshold)
            add_forward_sum_block(handles.bd_ax, ii-1, circ_scl);
        end
        if any(abs(a(max(ii,2):end)) > options.CoefficientThreshold)
            add_feedback_sum_block(handles.bd_ax, ii-1, circ_scl);
        end
    end
    ...
    if any(abs(a(2:end)) > options.CoefficientThreshold)
        line(handles.bd_ax,[-2.5,-2-circ_scl*0.75],[0 0],'Color','k','LineStyle','-','Marker','>','MarkerFaceColor','k','MarkerIndices',2,'MarkerSize',10,'LineWidth',2.5,'MarkerEdgeColor','none');
    else
    ...
    line(handles.bd_ax,[2+circ_scl*0.5,2.5],[0 0],'Color','k','LineStyle','-','Marker','>','MarkerFaceColor','k','MarkerIndices',2,'MarkerSize',10,'LineWidth',2.5,'MarkerEdgeColor','none');
    % % Top Row % %
    if any(abs(a(2:end)) > options.CoefficientThreshold)
        text(handles.bd_ax,-2+circ_scl*0.75,0.1,'w[n]','Color','k','FontName','Tahoma','FontAngle','italic','Rotation',45);
    end
    line(handles.bd_ax,[-2+circ_scl*0.5,1-triangle_scl*0.5], [0 0],'Color','k','LineStyle','-','LineWidth',2);
    line(handles.bd_ax,[1+triangle_scl*0.65,2-circ_scl*0.65], [0 0],'Color','k','LineStyle','-','Marker','>','MarkerFaceColor','k','MarkerIndices',2,'MarkerSize',10,'LineWidth',2.5,'MarkerEdgeColor','none');
    % % Middle column % %
    for ii = 1:ord
        % Top-half middle column vertical %
        line(handles.bd_ax,[0, 0], [-(ii-1), -(ii-1)-0.5+block_scl*0.5],'Color','k','LineStyle','-','LineWidth',2);
        % Bottom-half middle column vertical %
        line(handles.bd_ax,[0, 0], [-ii+0.5-0.5*block_scl,-ii],'Color','k','LineStyle','-','LineWidth',2);
        % Feedback middle column to triangle horizontal %
        if abs(a(ii+1)) > options.CoefficientThreshold
            line(handles.bd_ax,[0, -1+0.5*triangle_scl], [-ii,-ii],'Color','k','LineStyle','-','LineWidth',2);
        end
        % Feedback triangle to sum horizontal %
        if abs(a(ii+1)) > options.CoefficientThreshold
            line(handles.bd_ax,[-1-0.65*triangle_scl, -2+circ_scl*0.65],[-ii, -ii],'Color','k','LineStyle','-','Marker','<','MarkerFaceColor','k','MarkerIndices',2,'MarkerSize',10,'LineWidth',2.5,'MarkerEdgeColor','none');
        end
        % Feedback sum to previous sum vertical %
        if any(abs(a((ii+1):end)) > options.CoefficientThreshold)
            line(handles.bd_ax,[-2, -2],[-ii+0.5*circ_scl, -(ii-1)-0.65*circ_scl],'Color','k','LineStyle','-','Marker','^','MarkerFaceColor','k','MarkerIndices',2,'MarkerSize',10,'LineWidth',2.5,'MarkerEdgeColor','none');
        end
        % Forward middle column to triangle horizontal %
        if abs(b(ii+1)) > options.CoefficientThreshold
            line(handles.bd_ax,[0, 1-0.5*triangle_scl], [-ii,-ii],'Color','k','LineStyle','-','LineWidth',2);
        end
        % Forward triangle to sum horizontal %
        if abs(b(ii+1)) > options.CoefficientThreshold
            line(handles.bd_ax,[1+0.65*triangle_scl, 2-circ_scl*0.65],[-ii, -ii],'Color','k','LineStyle','-','Marker','>','MarkerFaceColor','k','MarkerIndices',2,'MarkerSize',10,'LineWidth',2.5,'MarkerEdgeColor','none');
        end
        % Forward sum to previous sum vertical %
    ...
    function add_delay_block(ax, index, scl)
        yc = -1*index+0.5;
        rectangle(ax, 'Position', [-0.5*scl, yc-0.5*scl, scl, scl], ...
            'LineWidth', 1.25, 'LineStyle', '-', 'EdgeColor', 'k');
        text(ax, 0, yc, 'z^{-1}', 'FontName','Tahoma','Color','k',...
    ...
    function h_a_txt = add_feedback_gain_block(ax, index, value, scl)
        ...
        h_a_txt = text(ax, xc+scl/8, yc, sprintf('-a_{%d}=%5.3f',index,-value), ...
            'FontName','Tahoma','FontSize',6,'Color','r',...
            'HorizontalAlignment','center', ...
            'VerticalAlignment','middle');
    end

    function add_feedback_sum_block(ax, index, scl)
        xc = -2;
        yc = -1*index;
        rectangle(ax, ...
            ...
            'LineStyle', '-', ...
            'EdgeColor', 'k');
        text(ax, -2, yc, '+', 'FontName','Tahoma','Color','k',...
            'HorizontalAlignment','center','VerticalAlignment','middle');
    end
    ...
    function add_forward_sum_block(ax, index, scl)
        xc = 2;
        yc = -1*index;
        rectangle(ax, ...
            'Position', [xc-0.5*scl, yc-0.5*scl, scl, scl], ...
